function ipLookUp () {
  ...
  .then(
      function success(response) {
          ...
          var dist=Math.sqrt(Math.pow((response.lat-schools[0][1]),2)+Math.pow((response.lon-schools[0][2]),2));
          for (var i = 0; i < schools.length; i++) {
            if (Math.sqrt(Math.pow((response.lat-schools[i][1]),2)+Math.pow((response.lon-schools[i][2]),2))<dist){
              dist=Math.sqrt(Math.pow((response.lat-schools[i][1]),2)+Math.pow((response.lon-schools[i][2]),2));
              closest=schools[i];
            }
          }
